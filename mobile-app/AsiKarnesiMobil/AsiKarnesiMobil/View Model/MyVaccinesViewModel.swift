...
class MyVaccinesViewModel: ObservableObject {
    ...
    /// Aşı Gizliliği POST
    func setPrivacyFunc(vaccination_id: String, new_privacy: Int, completion: @escaping (Bool) -> Void){
        let params: [String: Any] = [
            "vaccination_id": vaccination_id,
            "new_privacy": new_privacy
        ]
        guard let url = URL(string: "http://127.0.0.1:5000/set-privacy") else {return}
    ...
    func getSelectedVaccinePrivacyStatus(completion: @escaping (Bool) -> Void){
        guard let url = URL(string: "http://127.0.0.1:5000/get-privacy?vaccination_id=\(selectedVaccineID)") else {return}
    ...
    func decodeJSON(completion: @escaping (Bool) -> Void){
        ...
        guard let url = URL(string: "http://127.0.0.1:5000/user/codes?google_id=123") else {return}
